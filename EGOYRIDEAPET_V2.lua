...
-- =====================================================================
-- EGOY RIDE A PET V2 — MENU UI (Neon Violet / Cyber / Side Tabs / Mobile)
-- BY: EGOY
-- Original features/logic above are unchanged. Only the menu UI is new.
-- =====================================================================
...
A.Colors={
    Surface      = Color3.fromRGB(12,4,26),
    Surface2     = Color3.fromRGB(18,6,38),
    Text         = Color3.fromRGB(240,224,255),
    Title        = Color3.fromRGB(58,0,110),
    TitleGlow    = Color3.fromRGB(155,0,255),
    Border       = Color3.fromRGB(180,60,255),
    Row          = Color3.fromRGB(28,8,52),
    RowAlternate = Color3.fromRGB(20,4,40),
    Muted        = Color3.fromRGB(178,148,220),
    Accent       = Color3.fromRGB(150,0,255),
    Accent2      = Color3.fromRGB(210,80,255),
    Button       = Color3.fromRGB(46,10,86),
    Good         = Color3.fromRGB(215,150,255),
    Neon         = Color3.fromRGB(200,80,255),
    NeonSoft     = Color3.fromRGB(120,20,220),
    TabIdle      = Color3.fromRGB(22,4,42),
    TabActive    = Color3.fromRGB(130,0,220),
}
...
A.Page="Farm"
A.PreviewHeight=140
A.RowHeight=A.UIS.TouchEnabled and 40 or 30
A.TitleHeight=A.UIS.TouchEnabled and 46 or 34
A.TabWidth=A.UIS.TouchEnabled and 92 or 88
A.UpdateLabels={}
A.FilterButtons={}
A.Options.MenuKey=Enum.KeyCode.RightControl

local function neonize(obj,thickness,transparency)
    local s=Instance.new("UIStroke")
    s.Color=A.Colors.Neon
...
function A:Button(parent,text,callback,color)
    local button=self:Make("TextButton",parent,{Name=text:gsub("[^%w]",""),Text=text,
        ...
        BackgroundTransparency=0.15,BorderSizePixel=0,TextColor3=self.Colors.Text,
        Font=Enum.Font.Code,TextSize=14,AutoButtonColor=true,TextWrapped=true})
    self:Make("UICorner",button,{CornerRadius=UDim.new(0,6)})
    neonize(button,1,0.5)
    self:Connect(button.Activated,callback)
    return button
end

function A:Section(parent,text)
    local holder=self:Make("Frame",parent,{Name="Section",Size=UDim2.new(1,0,0,26),BackgroundTransparency=1})
    local bar=self:Make("Frame",holder,{Name="Bar",Size=UDim2.fromOffset(4,18),Position=UDim2.fromOffset(0,4),
        BackgroundColor3=self.Colors.Neon,BorderSizePixel=0})
    self:Make("UICorner",bar,{CornerRadius=UDim.new(1,0)})
    local label=self:Text(holder,text,26)
...
function A:RefreshMenuButtons()
    ...
    if mobile then
        ...
        self.MobileMenu.BackgroundColor3=self.Panel.Visible and self.Colors.Accent or self.Colors.Title
    end
end
...
function A:LayoutMobileMenu()
    ...
    local pos=self.MobileMenuPositioned and Vector2.new(self.MobileMenu.Position.X.Offset,self.MobileMenu.Position.Y.Offset)
        or Vector2.new(self.Bounds.AbsoluteSize.X-84,12)
...
function A:BuildMobileMenu()
    ...
    self.MobileMenu.Size=UDim2.fromOffset(76,48)
    self.MobileMenu.TextSize=13
    self.MobileMenu.ZIndex=30
    self:Make("UICorner",self.MobileMenu,{CornerRadius=UDim.new(0,10)})
    neonize(self.MobileMenu,1.5,0.15)
...
function A:NumberInput(parent,key,label,step,minimum,maximum,source,integer)
    ...
    local input=self:Make("TextBox",group,{Name="Value",Position=UDim2.fromOffset(self.RowHeight+4,22),
        ...
        BackgroundTransparency=0.15,BorderSizePixel=0,Text=tostring(source[key]),ClearTextOnFocus=false,
        TextColor3=self.Colors.Text,Font=Enum.Font.Code,TextSize=14})
    self:Make("UICorner",input,{CornerRadius=UDim.new(0,6)})
    neonize(input,1,0.6)
...
function A:OpenPopup(anchor,title,items,selected,multi,onChange,searchable)
    ...
    local height=math.min(360,math.max(160,area.Y-24))
    ...
    local box=self:Make("Frame",layer,{Name="Options",Position=UDim2.fromOffset(x,y),
        Size=UDim2.fromOffset(width,height),BackgroundColor3=self.Colors.Surface2,
        BackgroundTransparency=0.1,BorderSizePixel=0,ZIndex=2})
    self:Make("UICorner",box,{CornerRadius=UDim.new(0,10)})
    neonize(box,1.5,0.15)
    local header=self:Make("TextLabel",box,{Name="Title",Size=UDim2.new(1,0,0,self.RowHeight),
        Text=title,TextSize=14,Font=Enum.Font.GothamBold,TextColor3=self.Colors.Text,
        BackgroundColor3=self.Colors.Title,BackgroundTransparency=0.2,BorderSizePixel=0})
    self:Make("UICorner",header,{CornerRadius=UDim.new(0,10)})
    local top=self.RowHeight+5
    local search
    if searchable then
        search=self:Make("TextBox",box,{Name="Search",Position=UDim2.fromOffset(6,top),
            Size=UDim2.new(1,-12,0,self.RowHeight),Text="",PlaceholderText="Search...",
            ClearTextOnFocus=false,Font=Enum.Font.Code,TextSize=14,TextColor3=self.Colors.Text,
            PlaceholderColor3=self.Colors.Muted,BackgroundColor3=self.Colors.Row,
            BackgroundTransparency=0.15,BorderSizePixel=0})
        self:Make("UICorner",search,{CornerRadius=UDim.new(0,6)})
    ...
    local list=self:Make("ScrollingFrame",box,{Name="List",Position=UDim2.fromOffset(6,top),
        ...
        ScrollBarImageColor3=self.Colors.Border})
    ...
    for index,item in ipairs(items) do
        ...
        local row=self:Make("TextButton",list,{Name="Option"..index,Size=UDim2.new(1,-4,0,self.RowHeight),
            ...
            BackgroundColor3=self.Colors.Row,BackgroundTransparency=0.15,
            BorderSizePixel=0,AutoButtonColor=true,LayoutOrder=index})
        self:Make("UICorner",row,{CornerRadius=UDim.new(0,6)})
    ...
    local function footer(text,index,count,callback)
        local button=self:Make("TextButton",box,{Name=text,Position=UDim2.new((index-1)/count,6,1,-self.RowHeight-5),
            ...
            TextColor3=self.Colors.Text,BackgroundColor3=self.Colors.Button,
            BackgroundTransparency=0.15,BorderSizePixel=0})
        self:Make("UICorner",button,{CornerRadius=UDim.new(0,6)})
...
function A:FilterPanel(parent,kind)
    ...
    local search=self:Make("TextBox",parent,{Name=kind.."Search",Size=UDim2.new(1,0,0,self.RowHeight),
        BackgroundColor3=self.Colors.Row,BackgroundTransparency=0.15,BorderSizePixel=0,Text="",
        PlaceholderText="All egg names",ClearTextOnFocus=false,TextSize=14,Font=Enum.Font.Code,
        TextColor3=self.Colors.Text,PlaceholderColor3=self.Colors.Muted})
    self:Make("UICorner",search,{CornerRadius=UDim.new(0,6)})
...
function A:CreatePage(name)
    ...
    local page=self:Make("ScrollingFrame",self.Container,{Name=name:gsub(" ",""),
        Position=UDim2.fromOffset(0,0),
        Size=UDim2.new(1,0,1,-self.RowHeight-56),BackgroundTransparency=1,BorderSizePixel=0,
        CanvasSize=UDim2.fromOffset(0,0),AutomaticCanvasSize=Enum.AutomaticSize.Y,
        ScrollingDirection=Enum.ScrollingDirection.Y,ScrollBarThickness=3,
        ScrollBarImageColor3=self.Colors.Border,Visible=name==self.Page})
...
function A:ConfirmSellAll()
    ...
    self:Make("Frame",layer,{Name="PanelShade",Position=self.Panel.Position,Size=self.Panel.Size,
        AnchorPoint=self.Panel.AnchorPoint,BackgroundColor3=Color3.new(0,0,0),
    ...
    local box=self:Make("Frame",layer,{Name="Dialog",Position=UDim2.fromOffset(x,y),
        Size=UDim2.fromOffset(width,height),BackgroundColor3=self.Colors.Surface2,
        BackgroundTransparency=0.1,BorderSizePixel=0,ZIndex=2})
    self:Make("UICorner",box,{CornerRadius=UDim.new(0,10)})
    neonize(box,1.5,0.15)
    self:Make("TextLabel",box,{Name="Title",Size=UDim2.new(1,0,0,self.RowHeight),Text="Confirm Sell All",
        Font=Enum.Font.GothamBold,TextSize=14,TextColor3=self.Colors.Text,
        BackgroundColor3=self.Colors.Title,BackgroundTransparency=0.2,BorderSizePixel=0})
...
function A:ControlRefresh()
    for key,control in pairs(self.Controls) do
        ...
        if control.Toggle then
            ...
            control.Button.TextColor3=value and Color3.fromRGB(255,240,255) or self.Colors.Text
...
function A:SwitchPage(name)
    ...
    if self.Navigation then self.Navigation.Text=name.."  v" end
    if self.TabButtons then
        for tab,entry in pairs(self.TabButtons) do
            local active=tab==name
            entry.BackgroundColor3=active and self.Colors.TabActive or self.Colors.TabIdle
            entry.BackgroundTransparency=active and 0.05 or 0.35
            entry.TextColor3=active and Color3.fromRGB(255,235,255) or self.Colors.Muted
        end
    end
    self:RefreshUI()
end
...
function A:Layout(size)
    ...
    self:ClosePopup()
    local isMobile=self:IsMobileDevice()
    local targetW=isMobile and math.min(400,math.max(300,size.X-16)) or math.min(420,math.max(280,size.X-24))
    local targetH=self.Collapsed and self.TitleHeight or math.min(520,math.max(220,size.Y-24))
    self.Panel.Size=UDim2.fromOffset(targetW,targetH)
    if self.TabBar then
        self.TabBar.Size=UDim2.fromOffset(self.TabWidth,1)
        self.TabBar.Size=UDim2.new(0,self.TabWidth,1,-self.TitleHeight)
    end
    if self.Container then
        self.Container.Position=UDim2.fromOffset(self.TabWidth+6,self.TitleHeight+6)
        self.Container.Size=UDim2.new(1,-self.TabWidth-14,1,-self.TitleHeight-12)
    end
    ...
    local pos=self.Positioned and Vector2.new(self.Panel.Position.X.Offset,self.Panel.Position.Y.Offset)
        or Vector2.new(math.floor((size.X-targetW)/2),math.floor(size.Y*0.2))
    self:ClampPanel(pos)
...
-- ============================ Build GUI =============================
A.Gui=A:Make("ScreenGui",A.Player:WaitForChild("PlayerGui"),{Name="EGOY_RideAPetV2",ResetOnSpawn=false,
...
A.Panel=A:Make("Frame",A.Bounds,{Name="Panel",Size=UDim2.fromOffset(320,480),
    BackgroundColor3=A.Colors.Surface,BackgroundTransparency=0.18,BorderSizePixel=0,Active=true})
A:Make("UICorner",A.Panel,{CornerRadius=UDim.new(0,12)})
A.PanelStroke=A:Make("UIStroke",A.Panel,{Color=A.Colors.Neon,Thickness=2,Transparency=0.15,
    ApplyStrokeMode=Enum.ApplyStrokeMode.Border})

local panelGradient=Instance.new("UIGradient")
panelGradient.Color=ColorSequence.new({
    ColorSequenceKeypoint.new(0,Color3.fromRGB(155,0,255)),
...
panelGradient.Parent=A.Panel

-- Title bar
A.Title=A:Make("Frame",A.Panel,{Name="TitleBar",Size=UDim2.new(1,0,0,A.TitleHeight),
    BackgroundColor3=A.Colors.Title,BackgroundTransparency=0.35,BorderSizePixel=0})
A:Make("UICorner",A.Title,{CornerRadius=UDim.new(0,12)})
A:Make("UIStroke",A.Title,{Color=A.Colors.Neon,Thickness=1,Transparency=0.4})
...
A.Collapse.BackgroundTransparency=0.6
A.Collapse.TextSize=16

A.Header=A:Button(A.Title,"EGOY RIDE A PET V2",function() end,A.Colors.Title)
...
A.Header.Font=Enum.Font.GothamBold
A.Header.TextSize=A.UIS.TouchEnabled and 14 or 15
A.Header.TextColor3=Color3.fromRGB(255,225,255)
A.Header.BackgroundTransparency=1
A.Header.AutoButtonColor=false

A.Credit=A:Make("TextLabel",A.Title,{Name="Credit",Position=UDim2.new(1,-A.TitleHeight*2-94,0,0),
    Size=UDim2.fromOffset(94,A.TitleHeight),BackgroundTransparency=1,
    Text="BY:EGOY",TextColor3=A.Colors.Neon,Font=Enum.Font.GothamBold,
    TextSize=A.UIS.TouchEnabled and 12 or 13,TextXAlignment=Enum.TextXAlignment.Right})
...
A.Close.BackgroundTransparency=0.6
A.Close.TextColor3=Color3.fromRGB(255,180,240)
A.Close.TextSize=16

-- Side tab bar
A.TabBar=A:Make("Frame",A.Panel,{Name="TabBar",Position=UDim2.fromOffset(0,A.TitleHeight),
    Size=UDim2.new(0,A.TabWidth,1,-A.TitleHeight),BackgroundColor3=A.Colors.Surface2,
    BackgroundTransparency=0.55,BorderSizePixel=0})
A:Make("UIListLayout",A.TabBar,{Padding=UDim.new(0,3),SortOrder=Enum.SortOrder.LayoutOrder,
    HorizontalAlignment=Enum.HorizontalAlignment.Center})
A:Make("UIPadding",A.TabBar,{PaddingTop=UDim.new(0,6),PaddingBottom=UDim.new(0,6)})

A.TabButtons={}
local function buildTab(name,order)
    local tab=A:Make("TextButton",A.TabBar,{Name=name.."Tab",Size=UDim2.new(1,-10,0,A.RowHeight+4),
        BackgroundColor3=A.Colors.TabIdle,BackgroundTransparency=0.35,BorderSizePixel=0,
        Text=name,TextColor3=A.Colors.Muted,Font=Enum.Font.GothamBold,
        TextSize=A.UIS.TouchEnabled and 12 or 13,TextWrapped=true,AutoButtonColor=true,
        LayoutOrder=order})
    A:Make("UICorner",tab,{CornerRadius=UDim.new(0,7)})
    A:Make("UIPadding",tab,{PaddingLeft=UDim.new(0,4),PaddingRight=UDim.new(0,4)})
    neonize(tab,1,0.55)
    A.TabButtons[name]=tab
    tab.Activated:Connect(function() A:SwitchPage(name) end)
    return tab
end
for i,name in ipairs(A.PageNames) do buildTab(name,i) end

-- Content container (holds pages)
A.Container=A:Make("Frame",A.Panel,{Name="Container",
    Position=UDim2.fromOffset(A.TabWidth+6,A.TitleHeight+6),
    Size=UDim2.new(1,-A.TabWidth-14,1,-A.TitleHeight-12),BackgroundTransparency=1})

-- Hidden navigation button (kept for API compatibility, used by page picker if ever called)
A.Navigation=A:Button(A.Container,"",function() end,A.Colors.Row)
A.Navigation.Visible=false
...
A.FarmStats.TextColor3=A.Colors.Good
...
A.EggName.Font=Enum.Font.GothamBold
A.EggInfo={}
for index,key in ipairs({"Rarity","Luck","Weight","Mutation","Distance"}) do
    ...
    A:Make("UICorner",row,{CornerRadius=UDim.new(0,4)})
...
A:Section(pets,"Best Pet Placement")
...
A:Section(settings,"Anti AFK")
A:Toggle(settings,"AntiAFK","Anti-AFK")
A:Section(settings,"Menu")
...
A:Make("UICorner",A.Menu,{CornerRadius=UDim.new(0,10)})
neonize(A.Menu,1.5,0.15)
A:BuildMobileMenu()
